...
namespace ConsoleApp_CSV_Example
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var x = dbManager.Select<Country>("Select * FROM [world$]");
                var y = dbManager.Select("Select * from [world$]");
                ShowTable(y);

                var z = dbManager.Select<Country>("Select * from [world$] where alpha2 like 'n%'");
                ...
                //parameter examples
                var a = dbManager.Select<Country>("Select * from [world$] where alpha2 like ?", new DatabaseParameter("@p", "%a"));
                var b = dbManager.Select<Country>("Select * from [world$] where alpha2 like ? or alpha3 like ?", new DatabaseParameter("@p", "%a"), new DatabaseParameter("@p1", "n%"));
                ShowTable(dbManager.Select("Select * from [world$] where alpha2 like ? or alpha3 like ?", new DatabaseParameter("@p", "%a"), new DatabaseParameter("@p1", "n%")));

                //paged example
                int startPage = 1;
                var pagedCountries = dbManager.Select<Country>("Select * from [world$]", startPage, 10);
                ...
                while (pagedCountries.Count > 0)
                {
                    ...
                    startPage++;
                    pagedCountries = dbManager.Select<Country>("Select * from [world$]", startPage, 10);
                }
